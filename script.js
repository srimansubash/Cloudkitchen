...
//const placeOrderBtn = document.getElementById('place-order').addEventListener('click', () => {
  //if (!loggedIn) {
  ...
  // Existing code to show order popup
 // showOrderPopup();
//});
...
if (downloadPdfBtn) {
  downloadPdfBtn.addEventListener("click", () => {
    ...
    html2canvas(target).then((canvas) => {
      ...
      closeAndClearOrder(); // Clear cart, close popup, reset totals
    });
  });
}
...
const gmailLogin = document.getElementById('gmail-login');
...
function updateLoginIcon() {
  if (loggedIn) {
    loginBtn.innerHTML = `<i class="fa-solid fa-user-check text-yellow-500"></i> <span>Logged In</span>`;
  }
}

setInterval(updateLoginIcon, 500); // simple way to reflect login status
